...
load("C:/Users/Greg/Desktop/PrEP-Optimize/optim_res_table_costsharing.rda")
table_optim <- res

load("C:/Users/Greg/Desktop/PrEP-Optimize/optim_res_adh.rda")
adh_optim <- res

load("C:/Users/Greg/Desktop/PrEP-Optimize/optim_res_costsharing.rda")
...
\$669,200.00 per percentage point increase to PrEP Optimization Initiation Proportion (POIP)

\$5,650.80 per yearly adherence intervention capacity slot (POAC)

\$729.40 per retention intervention capacity slot (PORC)
...
Rather than assessing the overall cost per infection averted over the range of budget values, Figure 15 considers the average marginal increase cost per infection averted across the range of budget values. This can be conceptualized as the rate at which cost efficiency is worsening with diminished returns and other interactions.

```{r}
mcpia_plot <- ggplot(data = res_plot, aes(x = budget/1000, y = marg_cost_per_inf)) +
  geom_area() +
  labs(y = "Marginal increase in cost per infection averted ($)",
       x = "Budget (per $1,000)",
       title = "Figure 15") +
  theme_classic()+
  scale_x_continuous(breaks = round(seq(min(res_plot$budget/1000), max(res_plot$budget/1000), by = 500), 1), expand = c(0,0))
...
# load("C:/Users/Greg/Desktop/PrEP-Optimize/optim_res_table.rda")
# table_optim <- res
# 
...
#   gsub("@", "$", .)
knitr::include_graphics("C:/Users/Greg/Desktop/PrEP-Optimize/analysis/output_files/Outcome_table.png")
```
...
res_plot <- adh_optim %>%
  ...
  rowwise() %>%
  mutate(poip_budget_prop = poip * cost.i / budget,
         poac_budget_prop = poac * .54 * cost.a / budget,
         porc_budget_prop = porc * cost.r / budget,
         poip_budget = poip * cost.i,
         poac_budget = poac * .54 * cost.a,
         porc_budget = porc * cost.r)
